...
#' @param drop_missing_rows (optional) If set to \code{FALSE} and the \code{sub_table_cols} or \code{y_cols} are sparse factors then the missing factor levels introduce new rows filled filled with NAs. Otherwise no extra rows will be generated. Default value is \code{TRUE}.
...
#' @param data A data.frame that should be used for the creation of the cross table.
#' @param subLevel This is the recursive counter that gives the deepness of the recursive function execution. It points to the current sub heading level. So subLevel == 1 means that we look at the sub headings given by the different values in the data.frame column that has the name given in \code{sub_table_cols\[1\]}.
...
create_sub_table <- function(
    ...
    if (subLevel > length(sub_table_cols)) {
        ...
        subHeadingsCol <- data[[sub_table_cols[subLevel]]]
        ...
        for (heading in subHeadings) {
            ...
            if (!is.null(styledSubTable)) {
                ...
                if (!is.null(sub_heading_stylings)) {
                    styledSubHeading <- sub_heading_stylings[[subLevel]](styledSubHeading)
                }
                st <- styled_table(
                    st,
                    styledSubHeading,
